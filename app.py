...
# =========================
st.set_page_config(
    page_title="Cybersecurity Label Experiment v3",
    page_icon="🛡️",
    layout="centered"
)

CSV_FILE = "experiment_data_v3.csv"
...
def render_intro():
    ...
    with st.expander("研究參與說明"):
        st.write(
            "您將閱讀一段商品情境與產品資訊，接著回答幾個關於品牌判斷與價格評估的問題。"
            "您可隨時停止作答。"
        )
...
def render_survey():
    ...
    with st.form("experiment_form"):
        ...
        st.markdown("#### 訊號可信度（補充性檢查）")
        q_sc_1 = st.radio(
            "我認為該資安資訊是可信的。",
            scale_7, horizontal=True, index=None, key="sc1"
        )
        q_sc_2 = st.radio(
            "我認為該資安資訊具有專業性。",
            scale_7, horizontal=True, index=None, key="sc2"
        )

        st.markdown("#### 感知隱私風險（補充性檢查）")
        q_pr_1 = st.radio(
            "我擔心此產品可能造成隱私外洩。",
            scale_7, horizontal=True, index=None, key="pr1"
        )
        q_pr_2 = st.radio(
            "我認為使用此產品會讓我承擔隱私風險。",
            scale_7, horizontal=True, index=None, key="pr2"
        )
        ...
        q_bt_1 = st.radio(
            "我認為這個品牌是可靠的。",
            scale_7, horizontal=True, index=None, key="bt1"
        )
        q_bt_2 = st.radio(
            "我相信這個品牌會顧及使用者利益。",
            scale_7, horizontal=True, index=None, key="bt2"
        )
        q_bt_3 = st.radio(
            "我相信這個品牌不會讓我承擔不必要的資料安全風險。",
            scale_7, horizontal=True, index=None, key="bt3"
        )

        st.markdown("#### 購買意願（控制變數）")
        q_pi_1 = st.radio(
            "若我近期有需求，我會考慮購買此產品。",
            scale_7, horizontal=True, index=None, key="pi1"
        )
        q_pi_2 = st.radio(
            "整體而言，我對此產品有購買傾向。",
            scale_7, horizontal=True, index=None, key="pi2"
        )

        # C. 價格評估
        st.markdown("---")
        st.markdown("### C. 價格評估（主要結果）")
        st.write(
        ...
        privacy_concern = st.radio(
            "整體而言，我平時很重視個人隱私。(1=非常不同意，7=非常同意)",
            scale_7, horizontal=True, index=None, key="bg1"
        )
        digital_familiarity = st.radio(
            "我對智慧家電／聯網設備相當熟悉。(1=非常不同意，7=非常同意)",
            scale_7, horizontal=True, index=None, key="bg2"
        )
        ...
        if submit:
            ...
            if None in all_required_fields:
                ...
                record = {
                    ...
                    "mpl_0": mpl_answers[0],
                    "mpl_5": mpl_answers[5],
                    "mpl_10": mpl_answers[10],
                    "mpl_15": mpl_answers[15],
                    "mpl_20": mpl_answers[20],
                    "mpl_25": mpl_answers[25],
        ...
        if admin_input == ADMIN_PASSWORD:
            if os.path.exists(CSV_FILE):
                ...
                st.sidebar.download_button(
                    label="📥 下載資料 CSV",
                    data=csv_data,
                    file_name="experiment_data_v3.csv",
                    mime="text/csv"
                )
            else:
